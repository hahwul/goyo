...
`members` 숏코드는 팀 멤버들의 사진, 설명, 소셜 링크를 포함한 아름다운 멤버 디스플레이를 생성합니다. 그리드 및 가로 레이아웃을 모두 지원합니다.
...
| 파라미터 | 타입 | 기본값 | 설명 |
|---------|------|--------|------|
| `layout` | string | "grid" | 레이아웃 스타일: "grid" 또는 "horizontal" |
...
가로 레이아웃은 멤버를 스크롤 가능한 행으로 표시하며, 많은 멤버가 있는 팀 섹션에 완벽합니다.
